    ...
    if (questionType === 'parent') {
        ...
        if (options) {
            ...
            for (let i = 0; i < extractedOptions.length; i += 11) {
                ...
                const answer = { [question]: extractedOptions[i].answer ? extractedOptions[i].answer : null };
            ...
            for (let i = 0; i < extractedSubquestions.length; i += 11) {
                ...
                const answer = { [question]: extractedSubquestions[i].answer ? extractedSubquestions[i].answer: null };
                Object.assign(obj, answer);
            }
        }
    } else {
        const question = getQuestion(survey[key].question, null);
        Object.assign(obj, { [question]: survey[key].question ? survey[key].question: null });
        return;
    }
    return obj;
}
...
export default function transformSurveyResponse(stream, config) {
    return stream.map((obj) => { return transformSurvey(obj, config); });
}
