...
function transformSurveyTitle(data) {
    ...
    data.forEach((response) => {
        response.survey_title = stripSurveyTitle(response.survey_title);
    });
    return data;
}
...
function transformResponseMetadata(data) {
    data.forEach((response) => {
        delete response.survey_data;
        response.id = `${response.survey_id}_${response.id}`;
    });
    return data;
}
...
function transformSurvey(obj, config) {
    ...
    if (config.option === 'responses') {
        ...
        if (data.length > 0) {
            ...
            console.log('No responses. Length:', data.length);
            allData = data;
        }
    }
    return allData;
