...
function stripSurveyTitle(title) {
    ...
    let index;
    for (let i = 0; i < _title.length; i += 1) {
        let character = _title.charAt(i);
        if (!isNaN(parseInt(character)) || character === '-' || character === '_') {
            index = i + 1;
        } else {
            break;
        }
    ...
    let __title = _title.substr(index, _title.length);
    return __title;
}
...
function transformSurvey(obj, config) {
    ...
    if (config.option === 'responses') {
        ...
        const title = stripSurveyTitle(data.survey_title);
        data.survey_title = title;
